...
def forward_to_peer(peer_name: str, message: dict, my_url: str = "", my_token: str = "") -> dict:
    """Forward a message to a peer's daemon.

    The message dict should have: from, body, session_id (optional), type.
    If my_url and my_token are provided, includes auto-peer-discovery headers
    so the receiving Vex can add us as a peer automatically.
    ...
    if my_url and my_token:
        headers["X-Vex-Peer-Url"] = my_url
        headers["X-Vex-Peer-Token"] = my_token
        headers["X-Vex-Peer-Name"] = message.get("from", "")
